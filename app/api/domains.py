...
def _user_goals(user_id: str, domain_id: str) -> list:
    """Return the live goals list for a user+domain, seeded with demo data on first access."""
    _goal_store.setdefault(user_id, {})
    if domain_id not in _goal_store[user_id]:
        import copy
        _goal_store[user_id][domain_id] = copy.deepcopy(DEMO_GOALS.get(domain_id, []))
...
def _compute_progress(user_id: str, domain_id: str) -> DomainProgress:
    ...
    if total == 0:
        demo = DEMO_PROGRESS.get(domain_id, {})
        return DomainProgress(
            domain_id=domain_id,
            total_goals=demo.get("total", 0),
            completed_goals=demo.get("completed", 0),
            active_goals=demo.get("active", 0),
            current_streak=demo.get("streak", 0),
            weekly_checkins=demo.get("weekly_checkins", 0),
            completion_rate=demo.get("rate", 0),
        )
...
async def get_domain_checkins(
    ...
    # Back-fill with demo-style data if empty
    if not result:
        statuses = ["completed", "completed", "completed", "partial", "missed"]
        for i in range(days):
            date = datetime.utcnow() - timedelta(days=days - i - 1)
            result.append({
                "id": f"demo_{domain_id}_{i}",
                "domain_id": domain_id,
...
async def get_radar_data(
    ...
    for w in range(weeks):
        ...
        if week_snap:
            ...
            # Generate demo data for past weeks
            series.append({
                "week": week_str,
                "data": {d: random.randint(30, 90) for d in DOMAINS}
            })
...
async def get_snapshot_history(
    ...
    if not result:
        for w in range(weeks):
            ...
            if domain_id:
                result.append({
                    "id": f"demo_{domain_id}_{w}",
                    "domain_id": domain_id,
                    "week_start": week_str,
                    "completion_rate": random.randint(30, 90),
                    "active_goals": random.randint(2, 8),
                    "completed_goals": random.randint(0, 3),
                    "streak_days": random.randint(0, 30),
                    "checkins_week": random.randint(2, 7),
                })
            else:
                for d in DOMAINS:
                    result.append({
