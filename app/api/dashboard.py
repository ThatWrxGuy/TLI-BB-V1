...
async def get_dashboard(
    ...
    # Get user's actual goals (empty list if none created yet)
    goals = goals_db.get(user_id, [])
    
    # Get user's recommendations (empty list if none)
    recommendations = recommendations_db.get(user_id, [])
    
    # Get user's activity (empty list if none)
    activity = activity_db.get(user_id, [])
    ...
    overview = OverviewSummary(
        ...
        streak_days=0,  # New users have no streak
...
async def get_overview(
    ...
    # Get actual data - no demo data for new users
    goals = goals_db.get(user_id, [])
    recommendations = recommendations_db.get(user_id, [])
...
async def get_activity(
    ...
    # Get actual data - no demo data for new users
    activity = activity_db.get(user_id, [])
    return [ActivityItem(**a) for a in activity[:limit]]
...
async def get_enhanced_dashboard(
    ...
    # Domain scores - return empty for new users
    domain_scores_data = []
...
async def get_tasks(
    ...
    # Get actual tasks - no demo data for new users
